...
parser.add_argument("--torch", action="store_true", help="Start completely fresh by deleting all previous output")
...
if args.help_screen or not args.target_url:
    ...
    print("  python scanner.py target.htb --torch")
    sys.exit(0)
...
if not args.processors_only:
    if args.torch and os.path.exists(base):
        print("[*] Torch mode enabled: wiping everything in attacksurface/...")
        shutil.rmtree(base)

    os.makedirs(raw, exist_ok=True)

    # Clean up only the outputs that will be overwritten
    for index in selected:
        name, cmd_template = command_map[index]
        command = cmd_template.format(url=target, prefix=prefix, out=raw)
        out_files = [part for part in command.split() if part.startswith(f"{raw}/") or part.startswith(f"{raw}\\")]
        ...
        for filename in os.listdir(processors_dir):
            if filename.endswith(".py") and not filename.startswith("__"):
                ...
                try:
                    ...
                    print(f"[-] Processor error in {module_name}: {e}")
    else:
